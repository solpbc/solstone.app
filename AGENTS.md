...
| `billing.js`, `spb-billing.js`, `sme-billing.js`, `stripe.js`, `relay-grant.js`, `spb-entitlement.js`, `sme-entitlement.js`, `spb-broker.js` | The money path — Stripe checkout/portal/webhook (the webhook reconciles by the `metadata.service` tag checkout stamps, one reconciler per billed service), entitlement grants to `spl-relay`, the R2 credential broker. |
| `mcp-bridge.js`, `sme-service.js` | The bridge-token mint (it requires the owner's consent binding and a `sme_hosted` entitlement, checked before any hostname is allocated) and the module the service's owner-facing identity lands in. |
