...
| `billing.js`, `spb-billing.js`, `spa-billing.js`, `stripe.js`, `relay-grant.js`, `spb-entitlement.js`, `spa-entitlement.js`, `spb-broker.js` | The money path — Stripe checkout/portal/webhook (the webhook reconciles by the `metadata.service` tag checkout stamps, one reconciler per billed service), entitlement grants to `spl-relay`, the R2 credential broker. |
| `mcp-bridge.js`, `spa-service.js` | The bridge-token mint (it requires the owner's consent binding and a `spa_hosted` entitlement, checked before any hostname is allocated) and the module the service's owner-facing identity lands in. |
