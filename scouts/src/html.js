...
export function renderAdmin(scouts, feedback, news, adminEmail) {
  const scoutRows = scouts
    .map(
      (s) => `<tr>
    <td><code style="font-size:0.75rem;">${esc(s.did?.slice(0, 24))}...</code></td>
    <td>@${esc(s.handle)}</td>
    <td>${esc(s.email || '—')}</td>
    <td><span class="status-badge status-${s.status}">${s.status}</span></td>
    <td style="font-size:0.8rem;">${esc(s.use_case?.slice(0, 50) || '—')}</td>
    <td>
      ${s.status !== 'approved' ? `<form method="POST" action="/admin/scouts/${encodeURIComponent(s.did)}/approve" style="display:inline;"><button class="btn" style="padding:0.2rem 0.5rem;font-size:0.75rem;">approve</button></form>` : ''}
      ${s.status !== 'revoked' ? `<form method="POST" action="/admin/scouts/${encodeURIComponent(s.did)}/revoke" style="display:inline;"><button class="btn btn-secondary" style="padding:0.2rem 0.5rem;font-size:0.75rem;">revoke</button></form>` : ''}
      <form method="POST" action="/admin/scouts/${encodeURIComponent(s.did)}/token" style="display:inline;">
        <input type="text" name="gemini_key" placeholder="gemini key" style="width:140px;padding:0.2rem;font-size:0.75rem;margin:0;">
        <button class="btn" style="padding:0.2rem 0.5rem;font-size:0.75rem;">set key</button>
      </form>
    </td>
  </tr>`
    )
  ...
  const feedbackRows = feedback
    .map(
      (f) => `<tr>
    <td>@${esc(f.handle)}</td>
    <td><span class="status-badge">${esc(f.category)}</span></td>
    <td>${esc(f.body)}</td>
    <td style="font-size:0.8rem;">${esc(f.created_at?.slice(0, 16) || '')}</td>
  </tr>`
    )
    .join('');
  ...
  const newsRows = news
    .map(
      (n) => `<tr>
    <td>${esc(n.title)}</td>
    <td style="max-width:300px;">${esc(n.body?.slice(0, 100) || '')}</td>
    <td style="font-size:0.8rem;">${esc(n.posted_at?.slice(0, 16) || '')}</td>
  </tr>`
    )
    .join('');
  ...
  return layout(
    'admin — solstone scouts',
    `<div class="admin-container">
  <div class="nav">${SOL_WORDMARK.replace('width: 64px', '')}<h1 style="margin:0 0 0 0.5rem;">scouts admin</h1><span class="nav-right">${esc(adminEmail)}</span></div>

  <h2>scouts (${scouts.length})</h2>
  <table>
    <tr><th>did</th><th>handle</th><th>email</th><th>status</th><th>use case</th><th>actions</th></tr>
    ${scoutRows || '<tr><td colspan="6" style="color:#aaa;">no scouts yet</td></tr>'}
  </table>

  <div class="card">
    <h2 style="margin-top:0;">pre-approve a scout</h2>
    <form method="POST" action="/admin/scouts/pre-approve" style="display:flex;gap:0.5rem;align-items:end;">
      <div style="flex:1;">
        <label for="did_or_handle">DID or handle</label>
        <input type="text" id="did_or_handle" name="did_or_handle" placeholder="did:plc:... or handle.bsky.social" style="margin:0;">
      </div>
      <button type="submit" class="btn" style="white-space:nowrap;">pre-approve</button>
    </form>
  </div>

  <h2>feedback (${feedback.length})</h2>
  <table>
    <tr><th>scout</th><th>category</th><th>body</th><th>date</th></tr>
    ${feedbackRows || '<tr><td colspan="4" style="color:#aaa;">no feedback yet</td></tr>'}
  </table>

  <h2>news (${news.length})</h2>
  <table>
    <tr><th>title</th><th>body</th><th>posted</th></tr>
    ${newsRows || '<tr><td colspan="3" style="color:#aaa;">no news yet</td></tr>'}
  </table>
  ...
  <div class="card">
    ...
    <form method="POST" action="/admin/news">
      ...
      <input type="text" id="news-title" name="title" required placeholder="what's new?">
      <label for="news-body">body</label>
      <textarea id="news-body" name="body" required placeholder="details..."></textarea>
