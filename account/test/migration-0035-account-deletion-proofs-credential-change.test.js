...
import migration from '../migrations/0035_account_deletion_proofs_credential_change.sql?raw';
...
describe('migration 0035 account deletion proofs credential-change purpose', () => {
  beforeEach(async () => {
    await resetDb();
  ...
  it('preserves existing proof rows and admits credential-change without widening other coordinates', async () => {
    await applyThrough('0034_operator_label.sql');
    await workerEnv.DB.prepare(
      `INSERT INTO account_deletion_proofs (
       ...
       ) VALUES ('proof', 'account', 'session', 'export', 'otp', 1, 2, 1, 0, 3, 'code')`
    ).run();
    await runMigration(migration);

    await expect(workerEnv.DB.prepare(
      "SELECT * FROM account_deletion_proofs WHERE token_hash = 'proof'"
    ).first()).resolves.toMatchObject({ purpose: 'export', method: 'otp', verified: 1, attempt_count: 3 });
    await expect(workerEnv.DB.prepare(
      `INSERT INTO account_deletion_proofs (
         token_hash, account_id, session_id_hash, purpose, method, issued_at,
         expires_at, otp_code_hash
       ) VALUES ('credential-change-proof', 'account', 'session', 'credential-change', 'otp', 1, 2, 'code')`
  ...
  it('keeps the rebuilt table and lookup index byte-identical to canonical schema', async () => {
    await applyThrough('0035_account_deletion_proofs_credential_change.sql');
