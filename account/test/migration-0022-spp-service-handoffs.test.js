...
import migration from '../migrations/0022_spp_service_handoffs.sql?raw';
import { resetDb } from './helpers.js';

describe('migration 0022 spp service handoffs', () => {
  beforeEach(async () => {
    await resetDb();
    await installScoutPushSplSpbServiceHandoffs();
    await workerEnv.DB
      .prepare('INSERT INTO accounts (id, primary_email_id, created_at, last_signin_at) VALUES (?, NULL, ?, ?)')
      .bind('acct-migration-0022', 1_000, 1_000)
      .run();
  });

  it('broadens service_handoffs to spp, preserves rows, and recreates indexes idempotently', async () => {
    const spbRow = {
      handoff_hash: 'spb-hash',
      account_id: 'acct-migration-0022',
      service: 'spb',
    ...
    await expect(handoffRow('spb-hash')).resolves.toEqual(spbRow);
    ...
    await expect(tableSql()).resolves.toContain("service IN ('scout','push','spl','spb','spp')");

    await runMigration();
    await expect(insertHandoff({
      handoff_hash: 'spp-hash-2',
      account_id: 'acct-migration-0022',
      service: 'spp',
      payload_encrypted: 'spp-payload-2',
      created_at: 6_000,
      expires_at: 7_000,
      consumed_at: null,
    })).resolves.toBeUndefined();
    await expect(handoffRow('spb-hash')).resolves.toEqual(spbRow);
    await expect(handoffRow('spp-hash')).resolves.toMatchObject({ service: 'spp' });
    await expect(indexExists('idx_service_handoffs_account_id')).resolves.toBe(true);
    await expect(indexExists('idx_service_handoffs_expires_at')).resolves.toBe(true);
    await expect(tableSql()).resolves.toContain("service IN ('scout','push','spl','spb','spp')");
  });
...
async function installScoutPushSplSpbServiceHandoffs() {
