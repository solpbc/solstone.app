...
describe('session activity metadata', () => {
  ...
  it('bumps encrypted IP and capped user agent on an authenticated request', async () => {
    ...
    await worker.fetch(new Request('https://account.solstone.app/settings', {
      headers: {
  ...
  it('keeps the request alive when the activity update throws', async () => {
    ...
    const response = await worker.fetch(new Request('https://account.solstone.app/settings', {
  ...
  it('bounces a revoked session with the cleared cookie', async () => {
    ...
    const response = await worker.fetch(new Request('https://account.solstone.app/dashboard', {
      headers: { Cookie: session.cookie },
    }), testEnv);
  ...
  it('bounces a revoked session on settings with the cleared cookie', async () => {
    ...
    const response = await worker.fetch(new Request('https://account.solstone.app/settings/sessions', {
      headers: { Cookie: session.cookie },
    }), testEnv);
