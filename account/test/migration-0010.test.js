...
import migration from '../migrations/0010_service_handoffs_push.sql?raw';
import { resetDb } from './helpers.js';

describe('migration 0010 service handoffs push', () => {
  beforeEach(async () => {
    await resetDb();
    await installScoutOnlyServiceHandoffs();
    await workerEnv.DB
      .prepare('INSERT INTO accounts (id, primary_email_id, created_at, last_signin_at) VALUES (?, NULL, ?, ?)')
      .bind('acct-migration-0010', 1_000, 1_000)
      .run();
  });

  it('broadens service_handoffs to push, preserves rows, and recreates indexes idempotently', async () => {
    const scoutRow = {
      handoff_hash: 'scout-hash',
      account_id: 'acct-migration-0010',
      service: 'scout',
      payload_encrypted: 'scout-payload',
      created_at: 2_000,
      expires_at: 3_000,
      consumed_at: null,
    };
    await insertHandoff(scoutRow);
    ...
    await expect(handoffRow('scout-hash')).resolves.toEqual(scoutRow);
    ...
    await runMigration();
    await expect(insertHandoff({
      handoff_hash: 'push-hash-2',
      account_id: 'acct-migration-0010',
      service: 'push',
      payload_encrypted: 'push-payload-2',
      created_at: 6_000,
      expires_at: 7_000,
      consumed_at: null,
    })).resolves.toBeUndefined();
    ...
    await expect(indexExists('idx_service_handoffs_account_id')).resolves.toBe(true);
    await expect(indexExists('idx_service_handoffs_expires_at')).resolves.toBe(true);
    await expect(tableSql()).resolves.toContain("service IN ('scout','push')");
  });
});

async function installScoutOnlyServiceHandoffs() {
