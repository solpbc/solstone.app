-- migration 0020_spp_entitlement
-- Broaden entitlements.service from CHECK (service IN ('spl_hosted','spb_hosted'))
-- to CHECK (service IN ('spl_hosted','spb_hosted','spp_hosted')) and add the SPP binding schema hook.
--
...
-- 3. If entitlements already has CHECK (service IN ('spl_hosted','spb_hosted','spp_hosted')),
--    rerunning this file is safe: it rebuilds to the same schema and preserves all rows.
-- 4. spp_bindings uses CREATE TABLE IF NOT EXISTS and CREATE INDEX IF NOT EXISTS,
