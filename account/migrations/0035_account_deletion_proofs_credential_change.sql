-- migration 0035_account_deletion_proofs_credential_change
-- Add the 'credential-change' step-up purpose (passkey add/remove, email
-- add/remove/promote) without changing any other proof coordinate. Same
-- table-rebuild shape as 0032 (SQLite cannot ALTER a CHECK constraint).
--
...
--    lookup index below.
-- 3. If account_deletion_proofs already admits 'credential-change', this
--    migration completed; do not rerun it. Rebuilding an already-live proof
--    table can race a proof.
...
CREATE TABLE account_deletion_proofs_new (
  ...
  purpose TEXT NOT NULL CHECK (purpose IN ('delete', 'cancel', 'export', 'credential-change')),
