...
<p class="disclosure">by signing in, you agree to the <a href="/legal">terms</a>.</p>`,
...
<div class="card">
  ...
  <p class="disclosure">billed securely through Stripe. by subscribing, you agree to the <a href="/services/backup/terms">terms</a>.</p>
</div>
...
<div class="card">
  ...
  <p class="disclosure">billed securely through Stripe. complimentary for approved scouts. by subscribing, you agree to the <a href="/legal">terms</a>.</p>
</div>
...
<p class="notice">if you have a paid subscription, deleting your sign-in ends it when the 72-hour safety period below ends, and refunds the unused period to your payment method within 14 days.</p>
...
export function renderServicesTerms() {
  ...
  return layout({
    ...
    body: `${brandbar()}
<h1>${esc(title)}</h1>
<p class="meta"><em>${esc(`effective [ship date] · sol pbc, a Colorado public benefit corporation`)}</em></p>
...
<ul>
  <li><strong>${esc(`nothing here is required.`)}</strong>${esc(` solstone runs on devices you own, and your journal lives on one of them. three services take money today: private network, encrypted backup, and solstone.me. each has a way to do the same thing with sol pbc out of the path. confidential processing is in our tester program (scout) for now, and the part of notifications that reaches your phone isn't on yet. these terms cover your sign-in, the portal, and the parts sol pbc runs for you when you turn them on.`)}</li>
...
<ul>
  <li><strong>${esc(`everything is shown before you pay.`)}</strong>${esc(` each service has a flat price, plus any sales tax shown at checkout, per journal, not per device: one subscription covers one journal and every device you've paired with it (connected to it as one of yours). the price, the billing interval, that it renews automatically, and how to cancel are all shown to you at checkout, before we take any billing details, and you agree to all of it there with a separate, affirmative step before any charge; for solstone.me, section 14's permanent public record is shown and confirmed before any billing details too. after you subscribe we email you a confirmation that repeats those terms, our cancellation policy, and how to cancel, so you have a copy.`)}</li>
  <li><strong>${esc(`your subscription renews automatically`)}</strong>${esc(` at the end of each term, once a year on the annual plan or once a month on the monthly plan, at your plan's then-current price, using the payment method on file, `)}<strong>${esc(`until you cancel`)}</strong>${esc(`, or until a suspension runs to a renewal date (section 5). if a renewal payment fails, we email you and the portal tells you.`)}</li>
  <li><strong>${esc(`a renewal is never a surprise.`)}</strong>${esc(` on an annual plan we email you 25 to 40 days before each renewal with the date, the amount that will be charged, and a link to cancel. on a monthly plan we send the same email once a year, 25 to 40 days before the renewal that carries you past each full year.`)}</li>
  <li><strong>${esc(`we tell you before a price changes.`)}</strong>${esc(` a price change takes effect only on a renewal at least 45 days after we've emailed you, and you can cancel before it applies.`)}</li>
  ...
  <li><strong>${esc(`you keep what you paid for.`)}</strong>${esc(` when you cancel, the service keeps working until the end of the period you've already paid for, then stops. we don't prorate a cancellation you make on your own, and we don't claw back; sections 1, 5, 7, and 9 name the times we do refund unused time, and section 14 names one more. for confidential processing, what a plan includes within a period is part of the plan terms shown at checkout (section 13).`)}</li>
...
<p><strong>${esc(`what suspension can touch, and what it never can.`)}</strong>${esc(` sustained abuse can suspend your access to a service. it can never suspend your journal, your devices, or any path of your own: those are yours and don't run through us. if we suspend access we'll email you, say why, and say what it would take to restore it, and you can reply at support@solstone.app. a suspension never renews your subscription: if it lasts to your renewal date the subscription doesn't renew, and you can cancel it anytime from the billing portal. for encrypted backup, a suspension takes effect 30 days after we email you, so you can restore anything you offloaded to a device first (section 12), and your copy is never deleted while a suspension lasts. if we suspend a service you've paid for and we got it wrong, we refund the time you lost.`)}</p>
...
<ul>
  <li><strong>${esc(`it ends your subscriptions.`)}</strong>${esc(` if you still have time left, deleting your sign-in ends the subscription when the 72-hour safety period ends, and we refund the unused period the way section 9 does, to the payment method you used, within 14 days of the safety period ending.`)}</li>
  <li><strong>${esc(`it deletes the operated backup at once`)}</strong>${esc(`, with no 30-day lapse window. if you've turned on media offload, that copy is the only copy of that media, so restore it to a device first if you want to keep it. the deletion page says both and asks you to confirm.`)}</li>
...
<p>${esc(`we may update these terms. if a change is material, we'll notify you at least 30 days before it takes effect, with the date and how to cancel or turn the service off. for a change that takes effect at your next renewal, you can cancel before then if you don't agree. if a material change has to take effect mid-term, the notice says how to cancel for a prorated refund of the unused period (today: write to support@solstone.app before the date). if you keep using a service after a change takes effect, that's how you accept it; if you don't want to, turn the service off or delete your sign-in before then. the privacy policy is part of this agreement, so a material change to it gets the same notice. the cross-device part of notifications, once it's on, has its own turn-on screen, and section 15 says how we tell you about a change if you have no sign-in.`)}</p>
<p>${esc(`when we add a service, we add its section here and tell you. if we ever retire a service, we'll give you notice before it stops, refund any time you've paid for past that date, and the paths of your own stay. if sol pbc ever winds down, we'll give you as much notice as we can, with time to restore anything you offloaded, and Article 8 still governs where your data can go: anyone it goes to must take on covenants no less protective (section 6). we keep the current version posted here with the date it took effect.`)}</p>
...
<ul>
  ...
  <li>${esc(`these terms, the privacy policy, the disclosure you're shown when you turn on a service or a feature of one, the plan terms you're shown at checkout, and, if you're a scout, the program disclosure you acknowledged, are the whole agreement between us about the services. we keep a dated copy of every version of these terms, of the three documents they replaced, and of each turn-on disclosure, and we'll send you the version in force when you agreed if you ask. if these terms ever conflict with sol pbc's articles of incorporation, the articles govern.`)}</li>
  <li>${esc(`when we need to tell you something under these terms, we email the primary address on your sign-in; keep it current.`)}</li>
  <li>${esc(`if a court finds part of these terms unenforceable, the rest still applies. if we don't enforce something once, we can still enforce it later. sections 6, 7, 8, and 10, every refund promise in these terms (sections 1, 3, 5, 7, 9, and 14), the promises section 8 says aren't capped, what sections 11, 12, 13, and 14 say happens after a service stops, and the specific limits in sections 12 and 13, keep applying after a subscription or a sign-in ends, for as long as they're relevant.`)}</li>
...
<p><strong>${esc(`blind by construction.`)}</strong>${esc(` the relay passes encrypted bytes between your devices. it has no key to read them and keeps no copy of what flows through. sol pbc operates it and cannot read it.`)}</p>
...
<p><strong>${esc(`when it stops.`)}</strong>${esc(` cancel, or let it lapse, and the relay keeps working until the end of the period you paid for, then stops. a suspension under section 5 stops it while it lasts, and deleting your sign-in stops it when the safety period ends (section 7). your journal, your data, and your device pairings are untouched. the free paths keep working, and you're never locked out of your own journal by a billing state. re-subscribe anytime.`)}</p>
...
<ul>
  <li>${esc(`the operated storage keeps running through the end of the period you paid for, then stops. if a renewal payment fails, we email you and the portal tells you, and the clock below doesn't start until that period has ended. if your scout access ends, or you stop a service early under section 1 or 9, the storage stops that day and the clock starts then; once it has stopped, your journal can't read the backup until you subscribe. a suspension under section 5 is different: the storage stops, but your copy is kept and the clock doesn't start while the suspension lasts, and if it's lifted the storage turns back on with nothing lost.`)}</li>
...
<p><strong>${esc(`your use is metered, inside the container.`)}</strong>${esc(` the service keeps a count of how much confidential processing it has done for a journal, so it can manage capacity. apart from being down or full for a moment like any service (section 8), it can slow you down or say no on three grounds only: your access has ended (including a suspension under section 5), which the credential check catches when a channel opens; a paid plan's own terms say so (section 3), in the way those terms describe; or briefly under fair use, where for this service a no is temporary and clears on its own, not a suspension. your journal keeps thinking on its own either way. `)}<strong>${esc(`being metered is not being suspended`)}</strong>${esc(`: a limit doesn't reach your journal, your local processing, or your own key or endpoint, and the other paths above are always there.`)}</p>
...
<p><strong>${esc(`your journal's address, and the part of it that is permanent.`)}</strong>${esc(` when you turn this on, services.solstone.app mints an address for your journal: eight random characters with nothing of yours in them, followed by solstone.me. your journal then requests a real certificate for that address itself, the same kind secure websites use, and its key never leaves your machine; that certificate is how your agent can tell it's really your journal at the other end. issuing and renewing a certificate can be delayed, and there is a weekly ceiling on new certificates across everyone we serve, so turning on can land in a waiting state and a renewal can run late; the journal shows you which, and keeps trying. if an address never issues at all, write to support@solstone.app and we refund what you paid. `)}<strong>${esc(`and this is the part to read before you turn it on.`)}</strong>${esc(` certificates are published, by the authority that issues them, into public logs that anyone can search, and those logs are permanent by design. the logs gain an entry each time a certificate is issued or renewed for your address, and certificates are renewed regularly, so there is usually more than one. each entry shows that an address existed. none of them says whose, though an agent you connect knows the address is yours, and so do we until you delete your sign-in. and it cannot be taken back: it stays if you turn this off, it stays if you cancel, and it stays after you delete your sign-in. nobody can remove it, and that includes us. we say so on the screen where you turn this on, before you press the button and before any subscription is taken for it, and we ask you to confirm you've understood it. the authority your journal asks for the certificate is today Let's Encrypt, and the privacy policy describes those logs and what an entry in them shows.`)}</p>
...
<p><strong>${esc(`what's coming, and the rule it waits on.`)}</strong>${esc(` reaching a phone away from home needs a hop in between, because there's no way to reach a phone's push service without one. sol pbc has built that hop on Cloudflare's network, and it is not on for owners yet. it turns on only once we can say here exactly what the hop and your phone's push service (Apple's or Google's) will see, and only after our own review against Article 8 has cleared it. turning it on will be your choice, from a screen in the solstone app that says what it does and links here, and this section will say what the hop gets before it does. the hop is built so it holds no list of your devices and keeps no record of what it delivered; your journal keeps the list of devices you've turned notifications on for. if you turn it on without a sign-in, there's no email to reach you at, so any material change to these terms that applies to you, including any change to what the hop or your phone's push service gets, is told to you on that device, in the solstone app, before it takes effect. if it would send more than before, it first has to clear the same review against Article 8, and the cross-device part stays off on that device until you turn it on again there; if it sends less, it just takes effect.`)}</p>
...
<ul>
  ...
  <li><strong>${esc(`it can end.`)}</strong>${esc(` scout status is ours to grant and ours to end, and we can end it at any time as ordinary management of the program. ending it isn't a deletion: it ends the free access, and anything you pay for separately keeps running; you can always subscribe to a service you were getting as a scout. if encrypted backup was on for you as a scout, we email you when your access ends, and section 12's 30-day clock starts that day. once your access has ended your journal can't read the backup until you subscribe, so if you offloaded media, subscribing within those 30 days is the way to make it readable again, and then restore it to a device or keep the subscription. if solstone.me was on for you as a scout, your journal stops being reachable through the solstone.me relay when your access ends, and your address stays reserved for you on the same terms as section 14 sets out.`)}</li>
