...
// (adopted 2026-09-21 with operator approval; the sign-in-bound push consent retired with it)
...
export const SME_RELAY_EXPLANATION = "your journal holds the certificate key and is where the encrypted connection ends. the solstone.me relay currently forwards encrypted bytes and keeps no record of the traffic. sol pbc can see the network addresses, when a connection happens, and how much moved. Microsoft Azure runs the public machine and handles connection metadata. sol pbc controls the address and route. whoever controls that route or machine could obtain another valid certificate and change the route to read a future connection, including an agent's key. anyone holding that key can read what that agent may see until you disconnect the agent or revoke its key.";

export const SME_FREE_ROUTE_PARAGRAPHS = [
  "you never have to pay us to connect an agent on the same computer as your journal. with an updated journal, it can connect directly. from a different computer on your wifi or VPN, turn on 'agents on your network' in your journal's agents app and use an agent that accepts the journal's certificate. the agents app works with your own route or with solstone.me; solstone.me is the address, agents is where you manage what connects.",
  "a public hostname route needs a journal release that offers it; as of september 25, 2026, no released journal does. when one does, you will need your own hostname and certificate setup.",
  "some tunnels decrypt your traffic to move it. whoever runs one can read what your agent reads, and can reuse your agent's key to reach your journal as though they were it. a tunnel that only passes encrypted bytes avoids that routine decryption, but whoever runs its public machine could still get another valid certificate and change the route. when the public hostname route is available, keep control of your hostname and DNS, and publish the DNS record the journal shows to narrow who can get a valid certificate. once that route is released, solstone.me adds convenience but does not improve privacy over an owner-controlled tunnel that only passes encrypted bytes.",
];
...
<div class="card">
  ...
  <div class="grant">
    <div class="n">3</div>
    <div>
      <div class="gt">how the relay works today</div>
      <div class="gd">${SME_RELAY_EXPLANATION} details are in the <a href="https://solpbc.org/privacy#solstone-me">privacy policy</a>.</div>
    </div>
  </div>
  <div class="grant">
    <div class="n">4</div>
...
<div class="card">
  ...
  ${beat(IC_VAULT, 'not a tracking surface', "no analytics, no behavioral profiling, and no third party in the path but your phone's push service and, for iphone, Cloudflare, whose network our hop runs on. notifications never become a way to watch you.")}
</div>
<div class="card">
...
<div class="card">
  ...
  ${beat(IC_VAULT, 'how the relay works today', SME_RELAY_EXPLANATION)}
...
<div class="card">
  ...
  <div class="free-note" style="margin:14px 0 0">
    <h3>connect without solstone.me</h3>
    ${SME_FREE_ROUTE_PARAGRAPHS.map((part) => `<p>${esc(part)}</p>`).join('\n    ')}
  </div>
</div>
<p class="disclosure"><a href="/terms">terms</a></p>`,
  });
...
<div class="card">
  <h2>how the relay works today</h2>
  <p>${SME_RELAY_EXPLANATION}</p>
</div>
<div class="card">
...
<div class="card">
  <h2>connect without solstone.me</h2>
  ${SME_FREE_ROUTE_PARAGRAPHS.map((part) => `<p class="disclosure">${esc(part)}</p>`).join('\n  ')}
</div>`,
  });
}
...
// GENERATED by the terms renderer from the canonical services Terms of Service source
// (the region between the CANONICAL PUBLISHED TEXT delimiters). Regenerate and diff on any
// source change; never hand-edit the body between `${brandbar()}` and the closing template literal.
export function renderServicesTerms() {
  ...
  return layout({
    ...
    body: `${brandbar()}
<h1>${esc(title)}</h1>
<p class="meta"><em>${esc(`effective October 1, 2026 · sol pbc, a Colorado public benefit corporation`)}</em></p>
<p>${esc(`these terms cover `)}<strong>${esc(`services.solstone.app`)}</strong>${esc(`, the place you sign in, and the `)}<strong>${esc(`solstone services`)}</strong>${esc(` sol pbc runs for you there: private network, encrypted backup, confidential processing, solstone.me, and the part of notifications sol pbc runs. they're between you and sol pbc. the short version comes first; the details follow.`)}</p>
<h2>${esc(`the short version`)}</h2>
<ul>
  <li><strong>${esc(`nothing here is required.`)}</strong>${esc(` solstone runs on devices you own, and your journal lives on one of them. three services take money today: private network, encrypted backup, and solstone.me. each service's section describes what you can do with sol pbc out of the path, including where an alternative depends on your journal release. confidential processing is available to approved scouts, and notifications to your phone are off until you turn them on. these terms cover your sign-in, the portal, and the parts sol pbc runs for you when you turn them on.`)}</li>
  <li>${esc(`your journal is yours. private network carries encrypted bytes it can't read. encrypted backup holds encrypted blocks we have no key to. solstone.me gives your journal an address an agent you choose can reach. the relay currently forwards that connection encrypted; section 14 explains our control of the address and route. with confidential processing, our own model reads what you send, in the clear, only while it answers you, and verifiably keeps none of it; while it's on, your speech goes for transcription too, unless you turn that switch off. each service's section says what sol pbc, and the provider whose hardware it runs on, can see.`)}</li>
...
<ul>
  ...
  <li><strong>${esc(`the solstone services`)}</strong>${esc(` sol pbc runs for you: private network, the operated tier of encrypted backup, confidential processing, solstone.me, and the hop that carries notifications to an iphone (section 15). "operated" means sol pbc runs that part; each service's section describes the alternatives and their availability, except that hop, which section 15 explains;`)}</li>
...
<p>${esc(`each section below covers one service sol pbc runs, and only the part sol pbc runs. for private network, encrypted backup, confidential processing, and solstone.me, the alternatives with sol pbc out of the path are named first. each section says what is available and what setup it needs.`)}</p>
...
<p><strong>${esc(`you never have to pay us.`)}</strong>${esc(` the same private connection can be used without paying us. on the same network, your devices connect to your journal directly. or point solstone at your own VPN, Tailscale, or tunnel. or run the open-source relay yourself. all three are private by the same construction as the relay we run, because your devices encrypt to each other whatever path they take. `)}<strong>${esc(`the relay is convenience, never a privacy upgrade.`)}</strong></p>
...
<p><strong>${esc(`you don't need us on your own network.`)}</strong>${esc(` after updating to a journal release with local agent connections, agents can connect on the same computer; that route is on unless you turn it off. the network route can reach agents on your local network or VPN once you turn it on, but their app must accept the certificate your journal makes. some popular agent apps refuse that certificate in releases available as of september 25, 2026. a browser on another computer warns about it; the agents app shows how to check your journal's mark. older releases may not have either route. as of september 25, 2026, no released journal supports an owner-run public hostname. if a journal release adds that route, it will need your own hostname and certificate setup, and it won't require the solstone.me relay. some tunnels end the encryption to move traffic. their operator can read what your agent reads and use the credential your agent sends to reach your journal as that agent. if your journal supports your public hostname, a tunnel that only passes encrypted bytes can avoid that routine decryption. whoever controls its public endpoint may still be able to obtain another valid certificate and end a connection. if you use that route, keep control of its hostname, DNS and certificate account, and use the journal's DNS record that restricts new certificates to that account. solstone.me adds convenience; if the owner-hostname route is released, solstone.me will not improve privacy over an owner-controlled raw-byte path.`)}</p>
<p><strong>${esc(`how the relay works today.`)}</strong>${esc(` your journal holds its certificate key and ends the encrypted connection. the relay forwards encrypted bytes without keeping a database or record of what passes through it. its health records say something happened, never who it happened to. sol pbc controls the solstone.me address and public route, and Microsoft Azure operates the public machine. whoever controls that route or the public machine could obtain another valid certificate for the address and end a connection. ordinary HTTPS does not technically prevent that. a changed route could also expose the credential an agent sends through it, allowing its use within that agent's access until you disconnect that agent.`)}</p>
<p><strong>${esc(`your journal's address, and the part of it that is permanent.`)}</strong>${esc(` when you turn this on, services.solstone.app mints an address for your journal: eight random characters with nothing of yours in them, followed by solstone.me. your journal then requests a real certificate for that address itself, the same kind secure websites use, and its key never leaves your machine; that certificate lets your agent verify the solstone.me address, but by itself cannot prove that only your journal ends the connection. issuing and renewing a certificate can be delayed, and there is a weekly ceiling on new certificates across everyone we serve, so turning on can land in a waiting state and a renewal can run late; the journal shows you which, and keeps trying. if an address never issues at all, write to support@solstone.app and we refund what you paid. `)}<strong>${esc(`one thing about that address is public, and stays that way.`)}</strong>${esc(` like the certificates behind secure websites, each certificate for your address is listed in public certificate logs that nobody can edit, and a new entry is added each time one is issued or renewed. an entry shows that the address exists. none says whose, though an agent you connect knows the address is yours, and so do we until you delete your sign-in. it stays if you turn this off, cancel, or delete your sign-in, and nobody can remove it, including us. the authority your journal asks for the certificate is today Let's Encrypt, and the privacy policy says the same (`)}<a href="https://solpbc.org/privacy#solstone-me">${esc(`solstone.me`)}</a>${esc(`).`)}</p>
...
<p><strong>${esc(`what it is.`)}</strong>${esc(` notifications are built into the solstone app: free, turned on by you for each device, no sign-in needed. they give you a short heads-up on your devices when there's something worth a look.`)}</p>
<p><strong>${esc(`on the device, and on your phone.`)}</strong>${esc(` a heads-up can be a local notification on the device that shows it, and that never leaves the device. notifications from your journal can also reach your phone when you're away from home. those are off until you turn them on, phone by phone, in the solstone app, and your journal locks each one to a key only that phone and your journal hold before it leaves your machine.`)}</p>
<p><strong>${esc(`the hop, for iphone.`)}</strong>${esc(` reaching an iphone needs a hop, because only an app's own developer can hand a notification to Apple's push service. sol pbc runs that hop on Cloudflare's network. it takes the locked notification and your phone's push token, hands them to Apple with a fixed title and line, and keeps no record of them. android doesn't use it: your journal sends straight to your phone's push service. the privacy policy says what each one sees (`)}<a href="https://solpbc.org/privacy#notifications">${esc(`notifications`)}</a>${esc(`).`)}</p>
<p><strong>${esc(`not a tracking surface.`)}</strong>${esc(` no analytics, no behavioral profiling, and no third party in the path but your phone's push service and, for iphone, Cloudflare, whose network our hop runs on. notifications never become a way to watch you: the profiling half of that is Article 8, and the rest is a promise we make here.`)}</p>
<p><strong>${esc(`turning it off.`)}</strong>${esc(` turn notifications off on any device, anytime, from the solstone app on that device. on a phone, that also takes it off your journal's list the next time the phone reaches your journal.`)}</p>
